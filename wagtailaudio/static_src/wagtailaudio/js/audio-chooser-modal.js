AUDIO_CHOOSER_MODAL_ONLOAD_HANDLERS = {
    'chooser': function(modal, jsonData) {
        var searchUrl = $('form.audio-search', modal.body).attr('action');
        ...
        function fetchResults(requestData) {
            $.ajax({
                ...
                success: function(data, status) {
                    $('#audio-results').html(data);
                    ajaxifyLinks($('#audio-results'));
                }
            });
        }
        ...
        $('form.audio-upload', modal.body).on('submit', function() {
        ...
        $('form.audio-search', modal.body).on('submit', search);
        ...
        function populateTitle(context) {
            ...
            fileWidget.on('change', function () {
                ...
                if (title === '') {
                    // The file widget value example: `C:\fakepath\audio.mp3'
